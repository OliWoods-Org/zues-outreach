...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  const pageTitle =
    ...
    (location.pathname.startsWith('/scripts') ? 'Script Library' : 'Siren AI');
      ...
      >
        <div className="px-6 pt-7 pb-1">
          <Link to="/" className="block group" onClick={() => setMobileNavOpen(false)}>
            <h1 className="text-xl font-bold tracking-tight logo-gradient">Siren</h1>
            <p className="text-[11px] text-zinc-500 mt-1 tracking-wide">Voice revenue OS</p>
          </Link>
        </div>
